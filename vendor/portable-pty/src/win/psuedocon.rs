...
impl PsuedoCon {
    ...
    pub fn spawn_command(
        ...
        // Keep the array and borrowed job alive until after the attribute list.
        // CreateProcessW assigns the job before running the child, and fails
        // without starting an unprotected child if the assignment is invalid.
        let jobs = job.map(|job| [job.as_raw_handle() as HANDLE]);
        let mut attrs = ProcThreadAttributeList::with_capacity(if jobs.is_some() { 2 } else { 1 })?;
