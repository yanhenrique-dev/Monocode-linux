...
pub trait SlavePty {
    ...
    /// Spawns a Windows child in the supplied job before any child code runs.
    /// Returns an error if this PTY implementation cannot assign a job atomically.
    #[cfg(windows)]
    fn spawn_command_in_job(
        &self,
        _cmd: CommandBuilder,
        _job: std::os::windows::io::BorrowedHandle<'_>,
    ) -> Result<Box<dyn Child + Send + Sync>, Error> {
        anyhow::bail!("This PTY does not support atomic Windows job assignment")
    }
}
