...
// recomputed skill/mention tokenization and reconciled hundreds of spans,
// starving the CSS transitions around the composer (the "travadas").
const ComposerHighlight = memo(function ComposerHighlight({
  text,
  names,
  mentions,
}: {
  ...
  const parts = useMemo(() => skillTextParts(text, names), [text, names]);
  return (
    <>
...
const MentionRuns = memo(function MentionRuns({
  text,
  mentions,
}: {
  text: string;
  ...
  const parts = useMemo(
    () => fileMentionParts(text, mentions),
    [text, mentions],
  );
  return (
    <>
